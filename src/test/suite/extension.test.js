...
describe("Extension Tests", function () {
	...
	it("should open sidebar view", async () => {
		...
		await vscode.commands.executeCommand("goodloops.plusButtonClicked")
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("goodloops.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("goodloops.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("goodloops")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("goodloops.disableBrowserTool", undefined, true)
	})
})
