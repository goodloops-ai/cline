...
export class ClineProvider implements vscode.WebviewViewProvider {
	...
	*/
		...
		webview.onDidReceiveMessage(
			async (message: WebviewMessage) => {
				switch (message.type) {
					...
					// 	if (message.text) {
					// 		const response = await fetch(`https://api.goodloops.dev/v1/mcp/marketplace/item?mcpId=${message.mcpId}`)
	...
	async subscribeEmail(email?: string) {
		...
		try {
			const response = await axios.post(
				"https://app.goodloops.dev/api/mailing-list",
				{
					email: email,
				},
				{
	...
	private async fetchMcpMarketplaceFromApi(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get("https://api.goodloops.dev/v1/mcp/marketplace", {
				headers: {
					"Content-Type": "application/json",
				},
			})
	...
	private async downloadMcp(mcpId: string) {
		try {
			...
			const response = await axios.post<McpDownloadResponse>(
				"https://api.goodloops.dev/v1/mcp/download",
				{ mcpId },
				{
