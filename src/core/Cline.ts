...
export class Cline {
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const disableBrowserTool = vscode.workspace.getConfiguration("goodloops").get<boolean>("disableBrowserTool") ?? false
		...
		const preferredLanguage = getLanguageKey(
			vscode.workspace.getConfiguration("goodloops").get<LanguageDisplay>("preferredLanguage"),
		)
		...
		if (mcpHub) {
			...
			if (xstateServer && xstateServer.server.status === "connected") {
				try {
					...
					await mcpHub.callTool("xstate", "set_task_id", { taskId: this.taskId })
