...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	const settings = {
		...
		"workbench.view.extension.goodloops.goodloops-dev-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.goodloops.goodloops-dev-ActivityBar",
			when: "viewContainer.workbench.view.extension.goodloops.goodloops-dev-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
			command: "goodloops.openInNewTab",
			when: "viewContainer.workbench.view.extension.goodloops.goodloops-dev-ActivityBar.enabled",
		},
	]
	...
	const args = [
		...
		"--command",
		"workbench.view.extension.goodloops.goodloops-dev-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.goodloops.goodloops-dev-ActivityBar');
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						const extension = vscode.extensions.getExtension('goodloops.goodloops-dev');
						if (!extension) {
						...
						await vscode.commands.executeCommand('workbench.view.extension.goodloops.goodloops-dev-ActivityBar');
